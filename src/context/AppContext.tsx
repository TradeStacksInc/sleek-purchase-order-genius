...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getCurrentPrices = useCallback((): Record<ProductType, number> => {
    const currentPrices: Record<ProductType, number> = {} as Record<ProductType, number>;
    
    // Get the most recent active price for each product type
  ...
  const logAIInteraction = useCallback((prompt: string, response: string): void => {
    const log = {
      ...
      details: `Prompt: ${prompt.substring(0, 50)}... Response: ${response.substring(0, 50)}...`,
  ...
  const getOrdersWithDeliveryStatus = useCallback((status: string): PurchaseOrder[] => {
    return purchaseOrders.filter(order => order.deliveryDetails?.status === status);
  }, [purchaseOrders]);
  
  const getOrdersWithDiscrepancies = useCallback((): PurchaseOrder[] => {
    return purchaseOrders.filter(order => {
      const offloading = order.offloadingDetails;
      return offloading && offloading.isDiscrepancyFlagged;
    });
  }, [purchaseOrders]);
  ...
  const updateOrderStatus = useCallback(async (id: string, status: OrderStatus, note: string = ''): Promise<boolean> => {
    ...
    // Add status history if it exists
  ...
  const completeDelivery = useCallback(async (orderId: string): Promise<boolean> => {
    ...
    const updatedDeliveryDetails = {
      ...
      status: 'delivered' as 'pending' | 'delivered' | 'in_transit',
      actualArrivalTime: now,
      destinationArrivalTime: now
    };
  ...
  const recordOffloadingDetails = useCallback((orderId: string, details: any): boolean => {
    ...
    const offloadingDetails: OffloadingDetails = {
      ...
      isDiscrepancyFlagged: details.isDiscrepancyFlagged,
    ...
    // Add a log entry
    addLog({
      action: 'offloading_recorded',
      entityType: 'purchase_order',
      entityId: orderId,
      poId: orderId,
      details: `Offloading recorded for Purchase Order ${order.poNumber}`
    });
    
    // If there's a discrepancy, create an incident
    if (offloadingDetails.isDiscrepancyFlagged) {
      addIncident({
        type: 'volume_discrepancy',
        description: `Volume discrepancy of ${offloadingDetails.discrepancyPercentage.toFixed(2)}% detected during offloading of Purchase Order ${order.poNumber}`,
        severity: offloadingDetails.discrepancyPercentage > 5 ? 'high' : 'medium',
        impact: 'negative',
        timestamp: new Date(),
        location: 'Offloading Bay',
        orderId: orderId,
        reportedBy: offloadingDetails.measuredBy,
        status: 'open'
      });
    }
    
    return true;
  }, [purchaseOrders, addLog, addIncident]);
  ...
  const startDelivery = useCallback(async (orderId: string): Promise<boolean> => {
    ...
    // Find driver and truck info for better logging
    ...
    const now = new Date();
    // Calculate estimated arrival (mock for demo)
    const estimatedHours = Math.floor(Math.random() * 8) + 4; // 4-12 hours
    const expectedArrival = new Date(now.getTime() + estimatedHours * 60 * 60 * 1000);
    
    // Update delivery details
    const updatedDeliveryDetails = {
      ...order.deliveryDetails,
      status: 'in_transit' as 'pending' | 'delivered' | 'in_transit',
      depotDepartureTime: now,
      expectedArrivalTime: expectedArrival,
      totalDistance: order.deliveryDetails.totalDistance || Math.floor(Math.random() * 500) + 100, // 100-600km mock distance
      distanceCovered: 0
    };
    ...
    addLog({
      ...
      entityId: orderId,
      poId: orderId,
      details: `Delivery started for Purchase Order ${order.poNumber}${driver ? ` with driver ${driver.name}` : ''}${truck ? ` using truck ${truck.plateNumber}` : ''}`
    });
    
    return true;
  }, [purchaseOrders, getDriverById, getTruckById, updateDriver, updateTruck, addLog]);
  
  const updateDeliveryStatus = useCallback((orderId: string, status: string): boolean => {
    const orderIndex = purchaseOrders.findIndex(order => order.id === orderId);
    if (orderIndex === -1) return false;
    ...
    addLog({
      ...
      entityId: orderId,
      poId: orderId,
      details: `Delivery status updated to ${status} for Purchase Order ${order.poNumber}`
    });
  ...
  const assignDriverToOrder = useCallback((orderId: string, driverId: string, truckId: string): boolean => {
    ...
    let deliveryDetails: DeliveryDetails = order.deliveryDetails || {
      id: uuidv4(),
      status: 'pending',
      driverId: '',
      truckId: '',
      depotDepartureTime: undefined,
    ...
    deliveryDetails = {
      ...
      truckId,
      vehicleDetails: `${truck.plateNumber} (${truck.model})`
    };
    ...
    addLog({
      ...
      entityId: orderId,
      poId: orderId,
      details: `Driver ${driver.name} and truck ${truck.plateNumber} assigned to Purchase Order ${order.poNumber}`
    });
